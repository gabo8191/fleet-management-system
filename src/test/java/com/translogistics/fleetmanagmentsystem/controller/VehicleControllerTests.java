...
public class VehicleControllerTests {
    ...
    // 14. Programar mantenimiento: errores de validación
    @Test
    void scheduleMaintenanceShouldReturnScheduleMaintenanceViewWhenValidationErrors() {
        when(bindingResult.hasErrors()).thenReturn(true);
        MaintenanceDto dto = new MaintenanceDto();

        String viewName = vehicleController.scheduleMaintenance(1L, dto, bindingResult);
        assertEquals("vehicles/schedule-maintenance", viewName);
        verify(maintenanceService, never()).logMaintenance(anyLong(), any(MaintenanceDto.class));
    }

    // 15. Programar mantenimiento: caso exitoso
    @Test
    void scheduleMaintenanceShouldRedirectWhenValid() {
        when(bindingResult.hasErrors()).thenReturn(false);
        MaintenanceDto dto = new MaintenanceDto();

        doNothing().when(maintenanceService).logMaintenance(1L, dto);

        String viewName = vehicleController.scheduleMaintenance(1L, dto, bindingResult);
        assertEquals("redirect:/vehicles/1/maintenances", viewName);
        verify(maintenanceService, times(1)).logMaintenance(1L, dto);
    }
}
